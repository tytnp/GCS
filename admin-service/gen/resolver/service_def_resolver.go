...
func init() {
	core.Context.TargetResolver = append(core.Context.TargetResolver, &ServiceDefResolver{})
}

type ServiceDefResolver struct {
	file        *ast.File
	ImportSpecs map[string]string
	target      *core.TargetStruct
}

func (r *ServiceDefResolver) Parse(target *core.TargetStruct) {
	r.target = target
	r.ImportSpecs = map[string]string{
		fmt.Sprintf("\"%s/service/base\"", core.Context.DstModName):             "service",
		fmt.Sprintf("\"%s/model/%s\"", core.Context.DstModName, target.PkgName): "model",
	}
	fset := token.NewFileSet()
	filename := filepath.Join(core.Context.DstModPath, "service", target.PkgName, "definition.go")
...
func (r *ServiceDefResolver) SearchInitFuncByParam(funcParam string) (bool, bool) {
	isFuncExist := false
	isParamExist := false
...
func (r *ServiceDefResolver) AddInitFuncByParam(isFuncExist bool, isParamExist bool) {
...
func (r *ServiceDefResolver) SearchStructType() bool {
	isExist := false
	ast.Inspect(r.file, func(n ast.Node) bool {
		switch x := n.(type) {
		case *ast.TypeSpec:
			if _, ok := x.Type.(*ast.StructType); ok {
				if x.Name.Name == (r.target.Name + "Service") {
					isExist = true
					return false
				}
			}
...
func (r *ServiceDefResolver) AddStructType() {
	r.file.Decls = append(r.file.Decls, &ast.GenDecl{
		Tok: token.TYPE,
		Specs: []ast.Spec{
			&ast.TypeSpec{
				Name: ast.NewIdent(r.target.Name + "Service"),
				Type: &ast.StructType{
...
func (r *ServiceDefResolver) SearchImportSpec(importName string) bool {
	isExist := false
	ast.Inspect(r.file, func(n ast.Node) bool {
		switch x := n.(type) {
		case *ast.ImportSpec:
...
func (r *ServiceDefResolver) AddImportSpec(importPath string, alias string) {
	var name *ast.Ident
	if alias != "" {
