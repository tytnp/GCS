...
const tableProps = reactive({
    ...
    fetchData: async () => {
        ...
        if(NaN != dictionaryID){
            tableProps.params.dictionaryID = dictionaryID
        }
        const apiRet = await http.post('/dictionary-detail/list', tableProps.params)
...
caller.remove = async (id) => {
    const apiRet = await http.post('/dictionary-detail/del',{'id':id})
    if(apiRet.ok){
        tableProps.fetchData()
        message.info("删除成功!")
    }
