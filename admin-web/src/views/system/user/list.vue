    ...
    <n-layout >
        ...
        <n-layout-content style="margin-top: 0px">
            ...
            <n-modal v-model:show="showModal">
                <gcs-data-select :columnsMetaData="roleColumnsMetaData" :columns="roleColumns" :query-url="'/role/list'"
                                 v-model:retVal="roleId"
                                 v-model:showModal="showModal"
                                 :operate-func="callbackFun"
                                 url-path='/system/role'
                >
                </gcs-data-select>
            </n-modal>
        </n-layout-content>
...
import {columnsMetaData as roleColumnsMetaData} from "../role/definition.js";
import {columns as roleColumns} from "../role/custom.js";
...
const tableProps = reactive({
    ...
    fetchData: async () => {
        ...
        const apiRet = await http.post('/user/list', tableProps.params)
...
caller.remove = async (id) => {
    const apiRet = await http.post('/user/del',{'id':id})
    if(apiRet.ok){
        tableProps.fetchData()
        message.info("删除成功!")
    }
...
const callbackFun = async (id)=>{
    const apiRet = await http.post('/user/grantRole',{'userId':userId.value,'roleId':id})
    console.log(apiRet)
    if(apiRet.ok){
        message.info("授权完成")
        tableProps.fetchData()
